...
export const getAllUsers = async (req, res) => {
  try {
    const { nombrePersonal, segmento, fechaDeAuditoria, cuenta, tipoDeGrupo, tipoGrupo, cuentaAuditor, situacionLaboral, emailPersonal, limit = 5, page = 1 } = req.query;
    ...
    if (fechaDeAuditoria) {
      const fechaInicio = moment(fechaDeAuditoria).startOf('day').toISOString();
      const fechaFin = moment(fechaDeAuditoria).endOf('day').toISOString();
    ...
    if (cuentaAuditor) {
      filter.cuentaAuditor = { $regex: cuentaAuditor, $options: "i" }; // Insensible a mayúsculas
    }
